...
exports.getAll = (Model, popOptions) => catchAsync(async (req, res, next) => {
  let filter = {}; // allow for nested GET reviews on tour (hacky way to do this)
  ...
  if (popOptions) {
    const populateArr = Array.isArray(popOptions) ? popOptions : [popOptions];
    populateArr.forEach(option => {
